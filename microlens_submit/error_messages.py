...
def get_parameter_suggestions(model_type: str, user_param: str) -> List[str]:
    ...
    suggestions = []
    # If the user_param matches a typo for any canonical param, suggest the canonical param
...
def get_higher_order_effect_suggestions(invalid_effect: str) -> List[str]:
    ...
    valid_effects = [
        "parallax", "finite-source", "lens-orbital-motion", 
        "xallarap", "gaussian-process", "stellar-rotation", 
    ...
    common_typos = {
        ...
        "finite-source": ["finite_source", "finite source", "finite-source", "FINITE-SOURCE"],
        "lens-orbital-motion": ["lens_orbital_motion", "lens orbital motion", "LENS-ORBITAL-MOTION"],
        "xallarap": ["xallarap", "XALLARAP", "xallarap"],
        "gaussian-process": ["gaussian_process", "gaussian process", "GAUSSIAN-PROCESS"],
        "stellar-rotation": ["stellar_rotation", "stellar rotation", "STELLAR-ROTATION"],
        "fitted-limb-darkening": ["fitted_limb_darkening", "fitted limb darkening", "FITTED-LIMB-DARKENING"],
    }
...
def format_cli_error_with_suggestions(error_message: str, context: Dict) -> str:
    ...
    if "model_type must be one of" in error_message:
        ...
        enhanced += "\n   microlens-submit add-solution EVENT123 1S2L --param t0=2459123.5 --param u0=0.1 --param tE=20.0 --param s=1.2 --param q=0.001 --param alpha=45.0"
