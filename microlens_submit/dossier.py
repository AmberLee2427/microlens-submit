...
def _generate_dashboard_content(submission: Submission, full_dossier_exists: bool = False) -> str:
    ...
    Args:
        ...
        full_dossier_exists: (ignored, always uses placeholder now)
    Returns:
...
def _extract_main_content_div(html: str, id: str) -> str:
    """Extract the main content div (bg-white shadow-xl rounded-lg, any order) and add a unique id."""
    match = re.search(
        r'(<div[^>]+class=[\'\"][^\'\"]*bg-white[^\'\"]*shadow-xl[^\'\"]*rounded-lg[^\'\"]*[\'\"][^>]*>)([\s\S]*?)</div>',
        html
    )
    if not match:
        print("[DEBUG] Could not find main content div. HTML starts with:\n", file=sys.stderr)
        print(html[:2000], file=sys.stderr)
        sys.stderr.flush()
        try:
            with open("dossier_extract_debug.html", "w", encoding="utf-8") as f:
                f.write(html[:2000])
        except Exception as e:
            print(f"[DEBUG] Failed to write debug HTML file: {e}", file=sys.stderr)
            sys.stderr.flush()
        raise ValueError("Could not find main content div in HTML")
    div_open = match.group(1)
    inner = match.group(2)
    if 'id=' not in div_open:
        div_open = div_open.replace('div ', f'div id="{id}" ', 1)
    # Remove logo from extracted content for full dossier report
    # Remove the first <img ...rges-pit_logo.png...> in the div
    inner = re.sub(r"<img[^>]*rges-pit_logo.png[^>]*>\s*", "", inner, count=1)
    return f'{div_open}{inner}</div>'


def _generate_event_content(event, submission):
    """Return only the main content div for an event, with id."""
    html = _generate_event_page_content(event, submission)
    return _extract_main_content_div(html, f"event-{event.event_id}")


def _generate_solution_content(solution, event, submission):
    """Return only the main content div for a solution, with id."""
    html = _generate_solution_page_content(solution, event, submission)
    return _extract_main_content_div(html, f"solution-{solution.solution_id}")


def _generate_full_dossier_report_html(submission, output_dir):
    """Generate a single HTML file concatenating all dossier sections with anchor navigation."""
    all_html_sections = []
    # Dashboard
    dash_html = _generate_dashboard_content(submission, full_dossier_exists=True)
    dash_main = _extract_main_content_div(dash_html, "dossier-dashboard")
    all_html_sections.append(dash_main)
    # Events and solutions
    for event in submission.events.values():
        event_main = _generate_event_content(event, submission)
        all_html_sections.append(event_main)
        for sol in event.get_active_solutions():
            sol_main = _generate_solution_content(sol, event, submission)
            all_html_sections.append(sol_main)
...
<body class="font-inter bg-rtd-background">
    ...
    # Rewrite links to anchors
    # index.html -> #dossier-dashboard
    html = re.sub(r'href=["\"]/?.?index.html["\"]', 'href="#dossier-dashboard"', html)
    # events/[event_id].html -> #event-[event_id]
    html = re.sub(r'href=["\"]/?.?events/([A-Za-z0-9_-]+)\.html["\"]', r'href="#event-\1"', html)
    # [event_id]/solutions/[solution_id].html -> #solution-[solution_id]
    html = re.sub(r'href=["\"]/?.?([A-Za-z0-9_-]+)/solutions/([A-Za-z0-9_-]+)\.html["\"]', r'href="#solution-\2"', html)
    # ../[event_id].html -> #event-[event_id]
    html = re.sub(r'href=["\"]/?.?.?/([A-Za-z0-9_-]+)\.html["\"]', r'href="#event-\1"', html)
    # ../../index.html -> #dossier-dashboard
    html = re.sub(r'href=["\"]/?.?.?/index.html["\"]', 'href="#dossier-dashboard"', html)
    # solutions/[solution_id].html (from event page) -> #solution-[solution_id]
    html = re.sub(r'href=["\"]/?.?solutions/([A-Za-z0-9_-]+)\.html["\"]', r'href="#solution-\1"', html)
    # Write the file
