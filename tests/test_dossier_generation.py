...
def main():
    ...
    run_command(
        f"microlens-submit init --team-name 'Test Team Alpha' --tier 'advanced' {project_dir}"
    )
    ...
    run_command(
        f"microlens-submit set-repo-url https://github.com/AmberLee2427/microlens-submit.git {project_dir}"
    )
    ...
    run_command(
        f"""microlens-submit add-solution EVENT001 1S1L {project_dir} \
        --param t0=2459123.5 \
        --param u0=0.15 \
        --param tE=20.5 \
    ...
    run_command(
        f"""microlens-submit add-solution EVENT001 1S2L {project_dir} \
        --param t0=2459123.5 \
        --param u0=0.12 \
        --param tE=22.1 \
        --param q=0.001 \
    ...
    run_command(
        f"""microlens-submit add-solution EVENT002 1S2L {project_dir} \
        --param t0=2459156.2 \
        --param u0=0.08 \
        --param tE=35.7 \
        --param q=0.0005 \
    ...
    run_command(
        f"""microlens-submit add-solution EVENT003 2S1L {project_dir} \
        --param t0=2459180.0 \
        --param u0=0.25 \
        --param tE=18.3 \
    ...
    if csv_file.exists():
        ...
        result = run_command(
            f"microlens-submit import-solutions {csv_file} --project-path {project_dir} --dry-run",
            check=False,
        )
        ...
        result = run_command(
            f"microlens-submit import-solutions {csv_file} --project-path {project_dir}",
            check=False,
        )
        if result.returncode == 0:
    ...
    result = run_command(
        f"microlens-submit validate-submission {project_dir}", check=False
    )
    if result.returncode == 0:
    ...
    for event_id in [
        ...
        result = run_command(
            f"microlens-submit list-solutions {event_id} {project_dir}"
        )
        print(result.stdout)
        ...
        if event_page.exists():
            with event_page.open("r", encoding="utf-8") as f:
                ...
                if event_id == "EVENT001":
                    ...
                    assert (
                        "simple_point_lens" in content
                    ), f"Alias 'simple_point_lens' not found in {event_id}.html"
                    assert (
                        "detailed_analysis" in content
                    ), f"Alias 'detailed_analysis' not found in {event_id}.html"
                    assert (
                        "binary_with_parallax" in content
                    ), f"Alias 'binary_with_parallax' not found in {event_id}.html"
                elif event_id == "EVENT002":
                    assert (
                        "caustic_crossing_binary" in content
                    ), f"Alias 'caustic_crossing_binary' not found in {event_id}.html"
                elif event_id == "EVENT003":
                    assert (
                        "binary_source_model" in content
                    ), f"Alias 'binary_source_model' not found in {event_id}.html"
                elif event_id == "OGLE-2023-BLG-0001":
                    # Check for CSV imported aliases
                    assert (
                        "simple_1S1L" in content
                    ), f"Alias 'simple_1S1L' not found in {event_id}.html"
                    assert (
                        "binary_parallax" in content
                    ), f"Alias 'binary_parallax' not found in {event_id}.html"
                elif event_id == "OGLE-2023-BLG-0002":
                    assert (
                        "finite_source" in content
                    ), f"Alias 'finite_source' not found in {event_id}.html"
                    assert (
                        "duplicate_test" in content
                    ), f"Alias 'duplicate_test' not found in {event_id}.html"
                elif event_id == "OGLE-2023-BLG-0003":
                    assert (
                        "missing_params" in content
                    ), f"Alias 'missing_params' not found in {event_id}.html"
                elif event_id == "OGLE-2023-BLG-0004":
                    assert (
                        "invalid_json" in content
                    ), f"Alias 'invalid_json' not found in {event_id}.html"
    ...
    if alias_file.exists():
        ...
        for expected_alias in expected_aliases:
            assert (
                expected_alias in alias_lookup
            ), f"Expected alias '{expected_alias}' not found in aliases.json"
    ...
    print(
        f"🔍 Original alias for solution {first_solution_id[:8]}...: '{original_alias}'"
    )

    # Edit the alias of the first solution
    run_command(
        f"""microlens-submit edit-solution {first_solution_id} {project_dir} \
    ...
    if updated_solution.alias == "updated_simple_lens":
        ...
        print(
            f"❌ Alias update failed: expected 'updated_simple_lens', got '{updated_solution.alias}'"
        )
    ...
    if old_alias_key in final_alias_lookup:
        if final_alias_lookup[old_alias_key] == first_solution_id:
            assert (
                False
            ), f"Old alias '{old_alias_key}' still maps to edited solution {first_solution_id[:8]}..."
        else:
            print(
                f"✅ Old alias '{old_alias_key}' exists but maps to different solution (this is correct)"
            )

    # The new alias key should map to our edited solution
    assert (
        new_alias_key in final_alias_lookup
    ), f"New alias '{new_alias_key}' not added to lookup table"
    assert (
        final_alias_lookup[new_alias_key] == first_solution_id
    ), f"New alias '{new_alias_key}' maps to wrong solution"
